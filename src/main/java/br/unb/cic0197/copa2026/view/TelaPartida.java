...
public class TelaPartida extends JPanel {
    ...
    private JPanel buildFormPanel() {
        ...
        List<String> estadioNomes = estadioRepository.findAll().stream()
                .map(Estadio::getNome)
        ...
        List<String> selecaoNomes = selecaoRepository.findAll().stream()
                .map(Selecao::getPais)
                .collect(Collectors.toList());
        ...
        List<String> arbitroNomes = arbitroRepository.findAll().stream()
                .map(Arbitro::getNome)
                .collect(Collectors.toList());
    ...
    private JPanel buildSearchPanel() {
        ...
        selecaoRepository.findAll().stream()
                .map(Selecao::getPais)
    ...
    private void salvarPartida() {
        ...
        if (arbitroNome != null && !arbitroNome.isBlank() && !arbitroNome.equals("Nenhum") && !arbitroNome.equals("Sem árbitros")) {
            arbitro = arbitroRepository.findByNome(arbitroNome);
        }
        ...
        if (partidaSelecionada == null) {
            ...
            repository.update(partidaSelecionada);
    ...
    private void excluirPartida() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            repository.delete(partidaSelecionada);
            atualizarTabela(repository.findAll());
            limparCampos();
    ...
    private void buscarPartidas() {
        ...
        List<Partida> resultado = repository.search(selecao, fase, data, arbitro);
    ...
    private void selecionarPartidaNaTabela(ListSelectionEvent event) {
        ...
        repository.findById(id).ifPresent(this::carregarPartidaNoFormulario);
    }
